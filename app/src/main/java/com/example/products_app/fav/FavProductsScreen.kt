...
fun FavProductsScreen( viewModel: FavProductsViewModel) {
    ...
    Scaffold (
        ...
        if (dataState.value != null){
            LazyColumn {
                item{
                    Text(text = "Favorites", textAlign = TextAlign.Center, fontSize = 20.sp, modifier = Modifier.fillMaxWidth().padding(5.dp))
                }
                items(dataState.value?.size ?: 0){ currentIndex ->
                    ProductRow(dataState.value?.get(currentIndex), R.drawable.baseline_remove_24, { viewModel.removeProduct(it)})
                }
            }
        }else{
